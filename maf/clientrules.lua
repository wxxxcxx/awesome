...
function border_control(c)
    ...
    if (coords.x < geometry.x + range and coords.y < geometry.y + range) or -- left top
        (coords.x > geometry.x + geometry.width - range and coords.y <
            geometry.y + range) or -- right top
        (coords.x < geometry.x + range and coords.y > geometry.y +
            geometry.height - range) or -- left bottom
        (coords.x > geometry.x + geometry.width - range and coords.y >
            geometry.y + geometry.height - range) then
        -- right bottom
...
local clientbuttons = gears.table.join(awful.button({}, 1, function(c)
    if not awful.rules.match(c,
                             {class = "jetbrains-studio", name = "^win[0-9]+$"}) then
        client.focus = c
    end
    c:raise()
    border_control(c)
...
local function placement(d, args)
    ...
    if d.transient_for then
        ...
        -- args.parent = nil
    elseif d.instance ~= nil and client.focus ~= nil and d.instance ==
        client.focus.instance then
...
module = {
    ...
    }, -- Floating clients.
    {
        rule_any = {
            ...
            class = {
                ...
                "Wpa_gui", "veromix", "xtightvncviewer", "obs", "Qq", "Peek",
                "Anki", "Dragon-drag-and-drop", "scrcpy"
            },
            name = {
                "Event Tester", -- xev.
                "win0" -- jetbrains
    ...
    {rule_any = {class = {}}, properties = {ontop = true}}, -- No border clients
    {
        ...
        rule_any = {
            ...
            instance = {
                "jetbrains-idea", "jetbrains-datagrip", "emacs", "code",
                "code-oss"
            }
        },
        properties = {tag = "work"}
    }, {rule_any = {class = {"VirtualBox Machine"}}, callback = create_tag}
}
-- Enpass
