...
_M.settings = {
	preview_box = beautiful.switcher_preview_box or true,
	preview_box_bg = beautiful.switcher_preview_box_bg or "#ddddddaa",
	preview_box_border = beautiful.switcher_preview_box_border or "#22222200",
	preview_box_fps = beautiful.switcher_preview_box_fps or 3,
	preview_box_delay = beautiful.switcher_preview_box_delay or 150,
	preview_box_title_font = beautiful.switcher_preview_box_title_font or {"sans", "italic", "normal"},
	preview_box_title_font_size_factor = beautiful.switcher_preview_box_title_font_size_factor or 1,
	preview_box_title_color = beautiful.switcher_preview_box_title_color or "#000000",
	client_opacity = beautiful.switcher_client_opacity or false,
	client_opacity_value_selected = beautiful.switcher_client_opacity_value_selected or 1,
	client_opacity_value_in_focus = beautiful.switcher_client_opacity_value_in_focus or 1,
	client_opacity_value = beautiful.switcher_client_opacity_value or 1,
	cycle_raise_client = beautiful.switcher_cycle_raise_client or true,
	client_bg_selected = beautiful.switcher_client_bg_selected or "#00000000",
	client_bg_normal = beautiful.switcher_client_bg_normal or "#00000000",
	client_border_color = beautiful.border_select or "#00000000"
}
...
function _M.preview()
	...
	local y = screen[mouse.screen].geometry.height - h - textboxHeight
		...
		_M.preview_widgets[i].draw = function(preview_widget, preview_wbox, cr, width, height)
			if width ~= 0 and height ~= 0 then
				...
				if c == _M.altTabTable[_M.altTabIndex].client then
					cr:set_source(gears.color(_M.settings.client_bg_selected))
					cr:rectangle(10, 10, width - 10, height - 10)
					cr:fill()
					c.border_color = _M.settings.client_border_color
				else
					cr:set_source(gears.color(_M.settings.client_bg_normal))
					cr:rectangle(10, 10, width - 10, height - 10)
					cr:fill()
				...
				sx = iconboxWidth / icon.width / 2
				sy = iconboxHeight / icon.height / 2
				tx = (w - titleboxWidth) / 2
				ty = h + titleboxHeight / 4
				...
				-- Draw titles
				tx = tx + iconboxWidth - (titleboxHeight / 4)
				ty = h + (textboxHeight + textHeight) / 2

				cr:set_source(gears.color(_M.settings.preview_box_title_color))
				cr:move_to(tx, ty)
				cr:show_text(text)
				cr:stroke()
