...
local function group_client(groups, list)
    for k, c in ipairs(groups) do
        groups[k] = {}
    end
    for _, c in ipairs(list) do
        local k = c.instance
...
local function new(args)
    ...
    local w = wibox.widget.base.make_widget_from_value(args.layout or wibox.layout.flex.horizontal)
end
