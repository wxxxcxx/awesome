...
_M.settings = {
    preview_box = true,
    preview_box_bg = beautiful.switcher_bg,
    preview_box_border = beautiful.switcher_border_color,
    preview_box_fps = 5,
    preview_box_delay = 100,
    preview_box_title_font = {'sans', 'italic', 'normal'},
    preview_box_title_font_size_factor = 1,
    preview_box_title_color = beautiful.switcher_fg,
...
_M.preview_wbox.border_width = beautiful.switcher_border_width
...
function _M.preview()
    ...
    local W = screen[mouse.screen].geometry.width - margin * 2 -- + 2 * _M.preview_wbox.border_width
    local w = W / n -- widget width
    local h = w * 0.75 -- widget height
    local textboxHeight = w * 0.125

    local x = screen[mouse.screen].geometry.x - _M.preview_wbox.border_width + margin
        ...
        _M.preview_widgets[i].draw = function(preview_widget, preview_wbox, cr, width, height)
            if width ~= 0 and height ~= 0 then
                ...
                if c == _M.altTabTable[_M.altTabIndex].client then
                    fontSize = bigFont
                    -- Overlays
                    cr:set_source(gears.color(beautiful.switcher_focus_bg))
                ...
                if cg.width > cg.height then
                    sx = scale * w / cg.width
                    sy = math.min(sx, scale * h / cg.height)
                else
                    sy = scale * h / cg.height
                    sx = math.min(sy, scale * h / cg.width)
                end
